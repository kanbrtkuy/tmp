...
**2026-07-05 Fable-5 复审纠错**：下面两个 LOSO 表是历史诊断表，不能作为
prompt-only construct-validity 主证据直接引用。`Prompt-baseline 模块最强点`
列包含 `cot_*` 等 CoT 位置，不等同于严格 prompt-only baseline；
`Trajectory - prompt gap` 列也不能按相邻两列 AUROC 直接相减解释。论文主文
若回应 prompt-classification 担忧，应优先使用 natural same-prompt runs 中
prompt-only TF-IDF `0.5000`、prompt/pre-CoT hidden `0.5000/约0.51`、以及
validation-selected CoT-position hidden `~0.70-0.79` 的表；本节 LOSO 表只
作为 source-family 历史诊断或 appendix 材料。
