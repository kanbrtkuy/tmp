#!/usr/bin/env bash
# Shared RunPod storage/token environment.
#
# This file intentionally contains only stage-agnostic settings: hot/cold
# storage roots, Hugging Face token loading, caches, and generic PyTorch env.
# Stage-specific wrappers should source runpod_stage{1,2,3,4}_env.sh instead
# of adding training/probe/steering assumptions here.

if [[ -n "${COT_SAFETY_BASE_ENV_SOURCED:-}" ]]; then
  return 0 2>/dev/null || exit 0
fi
export COT_SAFETY_BASE_ENV_SOURCED=1

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
export COT_SAFETY_REPO_ROOT="${COT_SAFETY_REPO_ROOT:-$(cd "${SCRIPT_DIR}/.." && pwd)}"
