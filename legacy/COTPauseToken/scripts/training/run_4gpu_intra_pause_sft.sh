...
echo "  PAUSE_KL_ENABLED=$PAUSE_KL_ENABLED pause_tokens=$PAUSE_KL_PAUSE_TOKENS continuation=$PAUSE_KL_CONTINUATION_WEIGHT emit=$PAUSE_KL_EMIT_WEIGHT emit_margin_weight=$PAUSE_KL_EMIT_MARGIN_WEIGHT stop=$PAUSE_KL_STOP_WEIGHT suppression=$PAUSE_KL_SUPPRESSION_WEIGHT suppression_type=$PAUSE_KL_SUPPRESSION_LOSS_TYPE pause_head=$PAUSE_KL_PAUSE_HEAD_ENABLED suppression_chunk=$PAUSE_KL_SUPPRESSION_CHUNK_SIZE invariant_check=$PAUSE_KL_POST_STEP_INVARIANT_CHECK invariant_interval=$PAUSE_KL_INVARIANT_CHECK_INTERVAL_STEPS"
