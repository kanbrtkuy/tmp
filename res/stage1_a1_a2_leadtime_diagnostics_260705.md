...
off-diagonal cells are hidden at much later horizons compared against text at
earlier horizons, which is the wrong direction for a useful early-warning
claim.

Note: diagonal cells are re-bootstrapped in the same-horizon summaries and the
lead-time matrices, so tiny CI differences between those files should not be
read as substantive inconsistencies.
...
2. **A1 remains useful only as a diagnostic.** Its k=8 advantage was
   score-pooling-specific — consistent with an ensembling/variance-reduction
   effect over per-position probe scores rather than extra extractable
   information — and does not support a main claim.
...
5. **Allowed claim scope is narrow.** Pooled hidden AUROCs stay well above
   chance (0.736-0.793), but above-chance in-distribution decodability is
   expected under teacher forcing and is not evidence by itself; the
   equal-horizon comparison shows no hidden-over-text information advantage.
