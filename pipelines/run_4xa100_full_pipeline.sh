...
PYTHONPATH=src python -m cot_safety.cli pipeline plan --config configs/experiment/stage2_intra_pause_format_only_8b_cot4_save50_max250_4xa100.yaml > "${COT_SAFETY_RUN_ROOT}/stage2_intra_pause_sft_plan.json"
PYTHONPATH=src python -m cot_safety.cli pipeline plan --config configs/experiment/stage2_model_comparison_eval_8b_cot4_ckpt250_3xidle.yaml > "${COT_SAFETY_RUN_ROOT}/stage2_model_comparison_eval_plan.json"
...
echo "Set RUN_STAGE2=1 to launch Stage 2 cot4 format-only SFT with pipelines/run_4xa100_stage2_sft.sh."
echo "Stage 2 comparison eval defaults to cot4 checkpoint-250."
